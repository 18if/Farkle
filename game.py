...
class Farkle:
  ...
  def __init__(self, playing = 2):
    self.SCORES = {
      1: {1: 100, 2: 200, 3: 1000, 4: 2000, 5: 2100, 6: 2200},
  ...
  def complicated_scores(self, rolls, die_num, player):
    c = Counter(self.new_list)
    b = c[die_num]
    die_score = self.SCORES[die_num][b]
    self.scores[player] += die_score
    print("Score this turn: {}".format(die_score))
  ...
  def reroll(self, player):
    while True:
      ...
      if question == 'y':    
        rolls = [random.randint(1,6) for n in self.remaining]
        print('Player {} Rolled: {}'.format(player+1, rolls))
      if rolls == []:
        return False
  ...
  def play(self, player):
    ...
    self.complicated_scores(self.rolls, die_num, player)
    print(self.scores)
    self.reroll(player)
